#!/bin/bash
#
# From: https://github.com/sjiveson/nfs-server-alpine/blob/master/nfsd.sh
#
# Make sure we react to these signals by running stop() when we see them - for clean shutdown
# And then exiting

if [ "${AFP2NFS_DEBUG}" == "true" ]; then
	set -x
fi

# Trap the SIGTERM signal and do a clean kill
trap "stop; exit 0;" SIGTERM SIGINT

BIN="/usr/local/bin/mount_afp"
USERNAME="Kevin%20Flynn"
PASSWORD=${AFP2NFS_TIME_CAPSULE_PASSWORD}
CREDENTIALS="${USERNAME}:${PASSWORD}"
SHARED_DIRECTORY="/srv/afp"
CHECK="${SHARED_DIRECTORY}/.com.apple.timemachine.supported"
TC_IP=${AFP2NFS_TIME_CAPSULE_IP}

${BIN} afp://${CREDENTIALS}@${TC_IP}/Data ${SHARED_DIRECTORY}
sleep 5
ls -arilh ${SHARED_DIRECTORY}

stop_afp()
{
  echo "umounting afpfs timemachine.."
  umount ${SHARED_DIRECTORY}
  pid0=`pidof afp`
  kill -TERM $pid0 > /dev/null 2>&1
}
...
stop()
{
  ...
  echo "SIGTERM caught, terminating afpfs and NFS process(es)..."
  stop_afp
